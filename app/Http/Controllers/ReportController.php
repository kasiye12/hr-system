...
class ReportController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $request->validate([
            ...
            'project' => 'nullable|integer|min:0',
    ...
    private function generateStandardExcel($date, $projects, $categories, $entries)
    {
        ...
        $row = 1;
        $totalCol = chr(ord('C') + $projects->count());
        ...
        $sheet->setCellValue('B' . $row, 'Job Title / Category');
        $col = 'C';
        foreach ($projects as $p) {
            $sheet->setCellValue($col . $row, $p->name);
        ...
        $sheet->getStyle('A2:' . $totalCol . $row)->applyFromArray([
            'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN]],
        ]);
        
        $writer = new Xlsx($spreadsheet);
        $filename = 'Standard_HR_Report_' . $date . '.xlsx';
        $tempFile = tempnam(sys_get_temp_dir(), 'std_');
        $writer->save($tempFile);
